...
void motor_torque(float* torque_right, float* torque_left, uint16_t* steer_angle){
    ...
    uint16_t angle;
    float torque_ratio;
    static float base_torque = 0;
    static uint8_t max_torque = 25;
    uint8_t user_mode = PIND | 0x7F;
	float cruise_speed = 15;
    float current_speed = 13;    
    float error = 0; 
	...
	//This variable is used for the ramping feature
    angle = 0; //get_angle();	//get steering angle reading from encoder
    //angle = 10;
    *steer_angle = angle;	//modify the global steering angle variable
    ...
    switch(user_mode){
        ...
        case 0x7F:
            ...
            if(base_torque > max_torque){
                base_torque = max_torque;
            }
            //We are turning right
            if(angle >= 0 && angle <= 2048){
                torque_ratio = ((-0.00031)*angle)+(0.99972);	//Calculate torque ratio
                *torque_right = base_torque*torque_ratio;	//Update right motor torque
                *torque_left = base_torque;			//Update left motor torque
            }
            //We are turning left
            else{
                torque_ratio = ((1.033)*log((double)angle))-(7.59);	//Log function takes a double so had to typecast
                *torque_left = base_torque*torque_ratio;		//Update left motor torque
                *torque_right = base_torque;				//Update right motor torque
            }
            break;
        //Cruise Mode
        case 0x03:
			...
			error = cruise_speed - current_speed;
    		...
    		//error_prior = error
   			 ...
   			 if(output < 0){
       			 base_torque = base_torque - output;
    		}
   			 else{
       			 base_torque = base_torque + output;
  			}
