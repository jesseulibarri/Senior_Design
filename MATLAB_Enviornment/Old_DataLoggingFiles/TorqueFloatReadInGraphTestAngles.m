...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    angle_dat = fread(s, 1, 'uint16')
%     if(~isempty(angle_dat)) %Make sure torque_data Type is Correct        
%         count = count + 1;    
%         time(count) = toc;    %Extract Elapsed Time        
%         %for displaying read angle
%         data(count) = angle_dat(1);
%         torque_dat = [0,0,0,0];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

        A = [toc, torque_dat(1), torque_dat(2), angle_dat];    %Create 4x1 array of current timestamp and both read torque values
        fprintf(fileID,'%6.6f,%8.8f,%8.8f,%i\r\n',A);   %Save current into the log file
        %end
